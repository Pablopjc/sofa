...
# Both glyphs are drawn 18pt tall with no padding, matching the weight of the
# system's own menu bar icons. (The armchair SVG's viewBox is cropped to the
# glyph, otherwise ~30% of the canvas is empty air and it renders too small.)
set -euo pipefail
cd "$(dirname "$0")"
OUT="../Resources"

echo "▸ Armchair (custom glyph)…"
rsvg-convert -h 18 tray-armchair.svg -o "$OUT/trayTemplate.png"
rsvg-convert -h 36 tray-armchair.svg -o "$OUT/trayTemplate@2x.png"
...
function run(argv) {
  ...
  render(242, 144, 144, argv[0] + '/traySofaTemplate.png');
  return 'ok';
}
EOF

sips -z 36 61 "$OUT/traySofaTemplate.png" --out "$OUT/traySofaTemplate@2x.png" > /dev/null
sips -z 18 30 "$OUT/traySofaTemplate.png" > /dev/null
